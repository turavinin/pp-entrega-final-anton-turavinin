...
int eRecaudacion_GetRecaudaMayor1000(eRecaudacion* eArr, int largoArr, int* contRecauda)
{
	int exitoFuncion = -1;
	int aux = 0;
	...
	if(eArr != NULL && largoArr > 0)
	{
		...
		for(i = 0; i < largoArr; i++)
		{
			if(eArr[i].importe >= 1000 && eArr[i].estado.id == 3)
			{
				aux++;
			}
		}
...
int eRecaudacion_Eliminar(eRecaudacion* eRecaudaArr, eContribuyente* eContriArr, int largoRecauda, int largoContri)
{
	...
	if(exitoLocal == 0)
	{
		eRecaudacion_GetRecaudacionByContri(eRecaudaArr, largoRecauda, idContribuyente, posRecaudacion, &largoPosicionesRecauda);
