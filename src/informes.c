...
void printContribuyentesMayorFinanciar(eRecaudacion* recaudaArr, eContribuyente* contriArr, int largoArr)
{
...
void printRecaudacionesMayor1000(eRecaudacion* recaudaArr, int largoArr)
{
	...
	printf("\n|La cantidad de recaudaciones en estado 'saldado' y con un importe mayor a 1000 es %d:", cantidad);
}


// PUNTO D
void printContribuyentesFebrero(eRecaudacion* recaudaArr, eContribuyente* contriArr, int largoArr)
{
	int exitoLocal = -1;
	int i;
	int posicionContribuyente;
	...
	if(exitoLocal == 0)
	{
		...
		{
			printf("\n-----------------------------------------------------------------------------------------------------------------\n");
			printf("|CONTRIBUYENTES QUE PAGARON IMPUESTOS EN FEBRERO: ");
			for(i = 0; i < largoArr; i++)
			{
...
void imprimirContribuyenteCuilNombre(eContribuyente contribuyente)
{
	char nombreCompleto[MAX_CHAR];
	utn_FormatoNombreCompleto(contribuyente.nombre, contribuyente.apellido, nombreCompleto);
